...
/**
 * Performs "auto phrasing" on a token stream. Auto phrases refer to sequences
 * of tokens that are meant to describe a single thing and should be searched
 * for as such. When these phrases are detected in the token stream, a single
 * token representing the phrase is emitted rather than the individual tokens
 * that make up the phrase. The filter supports overlapping phrases.
 * <p/>
 * The Autophrasing filter can be combined with a synonym filter to handle cases
 * in which prefix or suffix terms in a phrase are synonymous with the phrase,
 * but where other parts of the phrase are not.
 */

final public class AutoPhrasingTokenFilter extends TokenFilter {
	...
	public boolean incrementToken() throws IOException {
		...
		// if (nextToken != null) System.out.println( "nextToken: " + new
		// String( nextToken ));
		if (nextToken == null) {
			if (lastValid != null) {
				emit(lastValid);
		...
		if (currentSetToCheck == null || currentSetToCheck.size() == 0) {
			...
			if (phraseMap.keySet().contains(nextToken, 0, nextToken.length)) {
				// get the phrase set for this token, add it to
				// currentSetTocheck
